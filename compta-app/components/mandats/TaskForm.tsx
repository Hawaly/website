...
export function TaskForm({ mandatId, task, mode, onSaved, onCancel }: TaskFormProps) {
  ...
  const [savedTaskId, setSavedTaskId] = useState<number | null>(task?.id || null);
  ...
  if (showVideoForm && savedTaskId) {
    return (
      <div className="space-y-6">
        <div className="bg-green-50 border-2 border-green-300 rounded-lg p-4 mb-4">
          <p className="text-green-900 font-semibold text-sm">
            ✓ Tâche créée avec succès ! Complétez maintenant les détails de la vidéo.
          </p>
        </div>
