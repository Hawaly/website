...
export async function generateSwissQrBill(data: QrBillData): Promise<Buffer> {
  return new Promise((resolve, reject) => {
    try {
      ...
      // Configurer le chemin vers les données de PDFKit (fichiers .afm)
      // @ts-ignore - Accès à une propriété interne de PDFKit
      if (PDFDocument.prototype && !PDFDocument.prototype.afmPath) {
        const afmPath = path.join(process.cwd(), 'node_modules', 'pdfkit', 'js', 'data');
        // @ts-ignore
        PDFDocument.prototype.afmPath = afmPath;
      }
