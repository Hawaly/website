...
DROP TABLE IF EXISTS public.pitch_deck_versions CASCADE;
DROP TABLE IF EXISTS public.pitch_deck_assets CASCADE;
DROP TABLE IF EXISTS public.pitch_deck_templates CASCADE;
...
DROP FUNCTION IF EXISTS public.update_pitch_deck_slide_count() CASCADE;
...
-- Table: pitch_decks (Enhanced with metrics and cache)
CREATE TABLE public.pitch_decks (
  ...
  -- Contenu (JSON structure complète)
...
CREATE INDEX idx_pitch_decks_template ON public.pitch_decks(template_name);

COMMENT ON TABLE public.pitch_decks IS 'Pitch decks et présentations commerciales avec métriques';
COMMENT ON COLUMN public.pitch_decks.slides IS 'Structure JSONB complète des slides avec tous les éléments';
COMMENT ON COLUMN public.pitch_decks.slide_count IS 'Nombre total de slides (calculé automatiquement)';
COMMENT ON COLUMN public.pitch_decks.tags IS 'Tags pour filtrage et recherche';

-- Table: pitch_deck_assets (NEW)
CREATE TABLE public.pitch_deck_assets (
  id SERIAL PRIMARY KEY,
  pitch_deck_id INTEGER NOT NULL REFERENCES public.pitch_decks(id) ON DELETE CASCADE,
...
CREATE INDEX idx_pitch_deck_assets_deck ON public.pitch_deck_assets(pitch_deck_id);
CREATE INDEX idx_pitch_deck_assets_slide ON public.pitch_deck_assets(pitch_deck_id, slide_index);

COMMENT ON TABLE public.pitch_deck_assets IS 'Assets (images, fichiers) utilisés dans les pitch decks';

-- Table: pitch_deck_templates (NEW)
CREATE TABLE public.pitch_deck_templates (
  id SERIAL PRIMARY KEY,
...
CREATE INDEX idx_pitch_deck_templates_category ON public.pitch_deck_templates(category);
CREATE INDEX idx_pitch_deck_templates_official ON public.pitch_deck_templates(is_official);

COMMENT ON TABLE public.pitch_deck_templates IS 'Templates prédéfinis pour pitch decks';

-- Table: pitch_deck_versions (NEW)
...
CREATE INDEX idx_pitch_deck_versions_deck ON public.pitch_deck_versions(pitch_deck_id);
CREATE INDEX idx_pitch_deck_versions_number ON public.pitch_deck_versions(pitch_deck_id, version_number);
...
CREATE TRIGGER pitch_deck_assets_updated_at BEFORE UPDATE ON public.pitch_deck_assets FOR EACH ROW EXECUTE FUNCTION public.set_timestamp();
CREATE TRIGGER pitch_deck_templates_updated_at BEFORE UPDATE ON public.pitch_deck_templates FOR EACH ROW EXECUTE FUNCTION public.set_timestamp();
...
-- Function: update slide_count automatically (NEW)
...
CREATE TRIGGER trigger_update_slide_count
  BEFORE INSERT OR UPDATE OF slides ON public.pitch_decks
  FOR EACH ROW
  EXECUTE FUNCTION public.update_pitch_deck_slide_count();
...
-- =========================================================
-- PART 15: SEED DATA - Pitch Deck Templates
-- =========================================================

-- Insert Urstory official templates
INSERT INTO public.pitch_deck_templates (name, description, category, is_official, slides_template, preview_image_url)
VALUES 
  (
    'Urstory Standard',
    ...
    'business',
    true,
    '[{"id":1,"title":"Page de garde","order":1,"content":{"type":"cover","elements":[{"id":1,"type":"heading","text":"Votre Titre Ici","level":"h1","color":"text-white","align":"center"},{"id":2,"type":"separator","style":"solid","color":"border-brand-orange","thickness":"border-4"},{"id":3,"type":"text","text":"Sous-titre ou description","fontSize":"text-xl","color":"text-slate-300","align":"center"},{"id":4,"type":"badge","text":"Powered by Urstory","color":"bg-brand-orange","textColor":"text-white"}]}},{"id":2,"title":"À propos","order":2,"content":{"type":"about","elements":[{"id":1,"type":"heading","text":"Qui sommes-nous ?","level":"h2","color":"text-brand-orange","align":"left"},{"id":2,"type":"columns","columnCount":2,"columns":[{"content":"Urstory est une agence digitale spécialisée."},{"content":"Notre mission : transformer vos idées en histoires."}]}]}},{"id":3,"title":"Services","order":3,"content":{"type":"services","elements":[{"id":1,"type":"heading","text":"Nos Services","level":"h2","color":"text-brand-orange","align":"center"},{"id":2,"type":"featureGrid","features":[{"icon":"Target","title":"Stratégie","description":"Stratégie digitale complète"},{"icon":"TrendingUp","title":"Croissance","description":"Augmentation visibilité"},{"icon":"Workflow","title":"Automatisation","description":"Optimisation processus"}]}]}},{"id":4,"title":"Résultats","order":4,"content":{"type":"results","elements":[{"id":1,"type":"heading","text":"Nos Résultats","level":"h2","color":"text-brand-orange","align":"center"},{"id":2,"type":"barChart","title":"Croissance moyenne","data":[{"name":"Q1","value":120},{"name":"Q2","value":180},{"name":"Q3","value":240},{"name":"Q4","value":320}],"dataKey":"value","color":"#f97316"}]}},{"id":5,"title":"Contact","order":5,"content":{"type":"contact","elements":[{"id":1,"type":"heading","text":"Contactez-nous","level":"h2","color":"text-brand-orange","align":"center"},{"id":2,"type":"callout","text":"Prêt à transformer votre présence digitale ?","calloutType":"info"},{"id":3,"type":"text","text":"www.urstory.ch\\ncontact@urstory.ch","fontSize":"text-lg","color":"text-slate-300","align":"center"}]}}]'::jsonb,
  ...
  (
    ...
    'social-media',
    true,
    '[{"id":1,"title":"Stratégie Social Media","order":1,"content":{"type":"cover","elements":[{"id":1,"type":"heading","text":"Stratégie Social Media 2024","level":"h1","color":"text-white","align":"center"},{"id":2,"type":"separator","style":"solid","color":"border-brand-orange","thickness":"border-4"},{"id":3,"type":"badge","text":"Par Urstory","color":"bg-brand-orange"}]}},{"id":2,"title":"Analyse","order":2,"content":{"type":"analysis","elements":[{"id":1,"type":"heading","text":"État des lieux","level":"h2","color":"text-brand-orange","align":"left"},{"id":2,"type":"comparison","title":"","items":[{"label":"Points forts","features":["Engagement actuel","Community active"]},{"label":"Axes amélioration","features":["Fréquence publication","Analytics"]}]}]}},{"id":3,"title":"Objectifs","order":3,"content":{"type":"objectives","elements":[{"id":1,"type":"heading","text":"Objectifs & KPIs","level":"h2","color":"text-brand-orange","align":"center"},{"id":2,"type":"metricCard","value":"+150%","label":"Croissance followers","trend":"up","color":"text-brand-orange"}]}},{"id":4,"title":"Plan action","order":4,"content":{"type":"action","elements":[{"id":1,"type":"heading","text":"Plan d''Action","level":"h2","color":"text-brand-orange","align":"left"},{"id":2,"type":"process","steps":[{"number":1,"title":"Audit","description":"Analyse complète"},{"number":2,"title":"Stratégie","description":"Définition axes"},{"number":3,"title":"Exécution","description":"Mise en place"}]}]}},{"id":5,"title":"Timeline","order":5,"content":{"type":"timeline","elements":[{"id":1,"type":"heading","text":"Roadmap","level":"h2","color":"text-brand-orange","align":"center"},{"id":2,"type":"timeline","items":[{"date":"Mois 1","title":"Audit","description":"Analyse et stratégie"},{"date":"Mois 2-3","title":"Création","description":"Production contenu"},{"date":"Mois 4+","title":"Optimisation","description":"Amélioration continue"}],"orientation":"vertical"}]}}]'::jsonb,
