...
import { ChevronLeft, ChevronRight, Calendar as CalendarIcon, Loader2, Users, TrendingUp } from 'lucide-react';
import { supabase } from '@/lib/supabaseClient';
import type { EditorialPost, Client } from '@/types/database';
import Link from 'next/link';

interface PostWithClient extends EditorialPost {
  client: Client;
}
...
export function AgencyEditorialCalendar() {
  ...
  const loadAgencyPosts = async () => {
    ...
    try {
      ...
      const strategyToClientMap = new Map(strategiesData.map(s => [s.id, s.client_id]));
      const calendarToClientMap = new Map(
        calendars.map(c => [c.id, strategyToClientMap.get(c.strategy_id)!])
      );
      ...
      const postsWithClient: PostWithClient[] = (postsData || [])
        .map(post => {
          const clientId = calendarToClientMap.get(post.calendar_id);
          const client = clientId ? clientsMap.get(clientId) : null;
          if (!client) return null;
          return { ...post, client };
        })
        .filter((post): post is PostWithClient => post !== null);

      setPosts(postsWithClient);
    } catch (error) {
  ...
  const filteredPosts = posts.filter(post => {
    if (filterClient !== 'all' && post.client.id !== filterClient) return false;
    if (filterPlatform !== 'all' && post.platform !== filterPlatform) return false;
    return true;
  });

  const getPostsForDate = (dateStr: string) => {
    return filteredPosts.filter(post => post.publication_date === dateStr);
  };
  ...
  const renderCalendarDays = () => {
    ...
    for (let day = 1; day <= daysInMonth; day++) {
      ...
      days.push(
        <div
          key={day}
          className={`min-h-[120px] border border-gray-200 p-2 bg-white hover:bg-gray-50 transition-colors overflow-hidden ${
                ...
                >
                  <div className="flex items-start justify-between gap-1">
                    <div className="flex-1 min-w-0 text-left">
                      <p className="text-[9px] text-gray-600 font-bold truncate">
                        {post.client.name}
                      </p>
  ...
  const stats = {
    ...
    clients: [...new Set(filteredPosts.map(p => p.client.id))].length,
  };
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 bg-gradient-to-r from-brand-orange to-brand-orange-light p-4 sm:p-6 rounded-xl shadow-lg text-white">
        <div className="flex items-center gap-3">
          <div className="p-3 bg-white/20 rounded-xl">
            <CalendarIcon className="w-6 h-6" />
          </div>
          <div>
            <h2 className="text-xl sm:text-2xl font-bold">Calendrier Éditorial Global</h2>
            <p className="text-sm text-white/90">Tous les posts de l'agence</p>
          </div>
        </div>
          ...
          <div className="flex items-start justify-between">
            <div className="flex-1">
              <Link 
                ...
                className="inline-block px-3 py-1 bg-white hover:bg-gray-50 border-2 border-gray-300 rounded-full text-sm font-bold mb-2 text-gray-900 transition-colors"
                onClick={(e) => e.stopPropagation()}
              >
                <Users className="w-4 h-4 inline mr-1" />
                {post.client.name}
              </Link>
              <div className={`inline-block ml-2 px-3 py-1 ${colors.bg} ${colors.text} rounded-full text-sm font-bold mb-2 border-2 ${colors.border}`}>
                {post.platform}
              </div>
