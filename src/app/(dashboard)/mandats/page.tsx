...
import { Search, Calendar, User, Briefcase, Filter, ChevronRight, Clock, XCircle, Sparkles } from "lucide-react";
      ...
      <main className="p-4 sm:p-6 lg:p-8 animate-fade-in">
        ...
        {isLoading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
            {filteredMandats.map((mandat, index) => (
              <motion.div
                key={mandat.id}
              ...
              >
                <Link
                  ...
                  className="block h-full bg-white rounded-2xl border border-slate-200/60 shadow-sm hover:shadow-xl p-6 transition-all duration-300 hover:border-orange-200"
                >
                  <div className="flex items-start justify-between mb-4">
                    <div className="flex-1">
                      <h3 className="font-bold text-slate-900 text-lg mb-2 group-hover:text-orange-600 transition-colors">{mandat.title}</h3>
                      <span className={`badge ${mandat.status === 'en_cours' ? 'badge-warning' :
                    ...
                    <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-orange-100 to-orange-200 flex items-center justify-center group-hover:from-orange-500 group-hover:to-red-500 transition-all">
                  ...
                  {mandat.mandat_type && (
                    <span className="inline-block px-2.5 py-1 text-xs font-medium bg-orange-50 text-orange-700 rounded-lg mb-3">
                      {mandat.mandat_type}
                    </span>
                  )}
