...
import { PremiumStrategyView } from "@/components/strategies/PremiumStrategyView";
import {
  ArrowLeft,
  Plus,
  Loader2,
...
export default function ClientStrategiesPage() {
  ...
  const [strategyEntities, setStrategyEntities] = useState<{
    personas: Persona[];
  ...
  async function handleViewStrategy(strategy: SocialMediaStrategy) {
    setSelectedStrategy(strategy);
    setIsLoadingEntities(true);
    setView('view');

    try {
      const entities = await getStrategyEntities(strategy.id);
      setStrategyEntities(entities);
    } catch (error) {
      console.error('Erreur lors du chargement des entités:', error);
      setStrategyEntities({ personas: [], piliers: [], kpis: [] });
    } finally {
      ...
      <>
        ...
        <main className="p-3 sm:p-6 lg:p-8 max-w-7xl mx-auto">
          ...
          <PremiumStrategyView
            ...
            personas={strategyEntities?.personas || []}
            piliers={strategyEntities?.piliers || []}
            kpis={strategyEntities?.kpis || []}
